...
func TestWalk_EdgeHandlers(t *testing.T) {
	files := newDeployFiles()
	...
	edgeHandlersDir, err := ioutil.TempDir(netlifyDir, "edge-handlers-dist")
	require.Nil(t, err)
	defer os.RemoveAll(edgeHandlersDir)

	err = ioutil.WriteFile(filepath.Join(edgeHandlersDir, "manifest.json"), []byte{}, 0644)
	require.Nil(t, err)
	...
	assert.NotNil(t, files.Files[".netlify/internal/edge-handlers/123456789.js"])
}

func TestWalk_PublishedFilesAndEdgeHandlers(t *testing.T) {
