...
Files for build can also be uploaded as a zipped site using one of these methods:
...
func (a *Client) MarkDevServerActivity(params *MarkDevServerActivityParams, authInfo runtime.ClientAuthInfoWriter) (*MarkDevServerActivityOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewMarkDevServerActivityParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "markDevServerActivity",
		Method:             "POST",
		PathPattern:        "/sites/{site_id}/dev_servers/{dev_server_id}/activity",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &MarkDevServerActivityReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	...
	success, ok := result.(*MarkDevServerActivityOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for markDevServerActivity: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
