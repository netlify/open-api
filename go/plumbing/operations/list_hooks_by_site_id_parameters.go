// Code generated by go-swagger; DO NOT EDIT.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// NewListHooksBySiteIDParams creates a new ListHooksBySiteIDParams object
// with the default values initialized.
func NewListHooksBySiteIDParams() *ListHooksBySiteIDParams {
	var ()
	return &ListHooksBySiteIDParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewListHooksBySiteIDParamsWithTimeout creates a new ListHooksBySiteIDParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewListHooksBySiteIDParamsWithTimeout(timeout time.Duration) *ListHooksBySiteIDParams {
	var ()
	return &ListHooksBySiteIDParams{

		timeout: timeout,
	}
}

// NewListHooksBySiteIDParamsWithContext creates a new ListHooksBySiteIDParams object
// with the default values initialized, and the ability to set a context for a request
func NewListHooksBySiteIDParamsWithContext(ctx context.Context) *ListHooksBySiteIDParams {
	var ()
	return &ListHooksBySiteIDParams{

		Context: ctx,
	}
}

// NewListHooksBySiteIDParamsWithHTTPClient creates a new ListHooksBySiteIDParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewListHooksBySiteIDParamsWithHTTPClient(client *http.Client) *ListHooksBySiteIDParams {
	var ()
	return &ListHooksBySiteIDParams{
		HTTPClient: client,
	}
}

/*ListHooksBySiteIDParams contains all the parameters to send to the API endpoint
for the list hooks by site Id operation typically these are written to a http.Request
*/
type ListHooksBySiteIDParams struct {

	/*Page*/
	Page *int32
	/*PerPage*/
	PerPage *int32
	/*SiteID*/
	SiteID string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithTimeout(timeout time.Duration) *ListHooksBySiteIDParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithContext(ctx context.Context) *ListHooksBySiteIDParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithHTTPClient(client *http.Client) *ListHooksBySiteIDParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithPage adds the page to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithPage(page *int32) *ListHooksBySiteIDParams {
	o.SetPage(page)
	return o
}

// SetPage adds the page to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetPage(page *int32) {
	o.Page = page
}

// WithPerPage adds the perPage to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithPerPage(perPage *int32) *ListHooksBySiteIDParams {
	o.SetPerPage(perPage)
	return o
}

// SetPerPage adds the perPage to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetPerPage(perPage *int32) {
	o.PerPage = perPage
}

// WithSiteID adds the siteID to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) WithSiteID(siteID string) *ListHooksBySiteIDParams {
	o.SetSiteID(siteID)
	return o
}

// SetSiteID adds the siteId to the list hooks by site Id params
func (o *ListHooksBySiteIDParams) SetSiteID(siteID string) {
	o.SiteID = siteID
}

// WriteToRequest writes these params to a swagger request
func (o *ListHooksBySiteIDParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if o.Page != nil {

		// query param page
		var qrPage int32
		if o.Page != nil {
			qrPage = *o.Page
		}
		qPage := swag.FormatInt32(qrPage)
		if qPage != "" {
			if err := r.SetQueryParam("page", qPage); err != nil {
				return err
			}
		}

	}

	if o.PerPage != nil {

		// query param per_page
		var qrPerPage int32
		if o.PerPage != nil {
			qrPerPage = *o.PerPage
		}
		qPerPage := swag.FormatInt32(qrPerPage)
		if qPerPage != "" {
			if err := r.SetQueryParam("per_page", qPerPage); err != nil {
				return err
			}
		}

	}

	// query param site_id
	qrSiteID := o.SiteID
	qSiteID := qrSiteID
	if qSiteID != "" {
		if err := r.SetQueryParam("site_id", qSiteID); err != nil {
			return err
		}
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
